...
const targetProxy = `  app.get(["/api/proxy-image", "/api/proxy/image"], async (req, res) => {
    try {
      ...
      if (!response.ok) {
        ...
        return res.status(200).send(\`
          <svg xmlns="http://www.w3.org/2000/svg" width="320" height="180" viewBox="0 0 320 180">
            ...
            <text x="50%" y="50%" dominant-baseline="middle" text-anchor="middle" fill="#A1A1AA" font-family="monospace" font-size="12">THUMBNAIL UNAVAILABLE</text>
          </svg>
        \`);
      }
...
const replProxy = `  app.get(["/api/proxy-image", "/api/proxy/image"], async (req, res) => {
    try {
      const rawUrl = (req.query.url || req.query.src) as string;
      ...
      const sendFallback = () => {
        ...
        const svg = \`
          <svg xmlns="http://www.w3.org/2000/svg" width="320" height="180" viewBox="0 0 320 180">
            ...
            <text x="160" y="142" dominant-baseline="middle" text-anchor="middle" fill="#A1A1AA" font-family="monospace" font-size="9" letter-spacing="1">SOURCE UNREACHABLE (404)</text>
          </svg>
        \`.trim();
...
content = content.replace(targetProxy, replProxy);
